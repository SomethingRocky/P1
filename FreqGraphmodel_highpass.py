...
# Læser csv'en
data = pd.read_csv("frequency_analysis_highpass.csv", sep=",", skiprows=20, encoding="utf-8", index_col=0)
...
model_frequency = 20 * np.log10(((omega * Cap * Res) / squareroot))
model_faze = np.degrees(np.arctan(1 / (omega*Res*Cap)))

max_squareroot = np.sqrt(1 + (omega * Cap * 1050) ** 2)
min_squareroot = np.sqrt(1 + (omega * Cap * 950) ** 2)
max_freq_deviation = 20 * np.log10(((omega * Cap * Res) / max_squareroot))
min_freq_deviation = 20 * np.log10(((omega * Cap * Res) / min_squareroot))

max_phase_deviation =np.degrees(np.arctan(1 / (omega*1050*Cap)))
min_phase_deviation =np.degrees(np.arctan(1 / (omega*950*Cap)))
